...
{
	int mahozNameLen, electionResultsSize;
	in.read(rcastc(&mahozNameLen), sizeof(mahozNameLen));
	if (!in.good())
	{
		cout << "Failed to load mahoz name len" << endl;
		exit(-1);
	}
	name.resize(mahozNameLen + 1);
	in.read(rcastc(&name[0]), mahozNameLen*sizeof(char));
	if (!in.good())
	{
